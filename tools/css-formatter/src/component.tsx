...
import type { WorkerAPI, CSSFormatterOptions } from './worker.ts';
...
export function Component({ ctx }: { ctx: ToolContext }) {
  ...
  const savedOptions = ctx.useLive<CSSFormatterOptions>('options');
  ...
  const [options, setOptions] = useState<CSSFormatterOptions>({
    printWidth: 80,
    tabWidth: 2,
    useTabs: false,
    singleQuote: false,
  ...
  const updateOptions = (updates: Partial<CSSFormatterOptions>) => {
  ...
  const formatCode = async (code: string, opts: CSSFormatterOptions) => {
    if (!worker || !code.trim()) {
      setOutput('');
      setError(null);
      return;
    ...
    try {
      const res = await worker.run((api) => api.formatCss(code, opts));
      setOutput(res);
      setError(null);
    } catch (e: any) {
  ...
  const handleOpenFile = async () => {
    try {
      ...
      const file = await ctx.filesystem.openFile({ accept: ['.css', '.scss', '.less', '*/*'] });
  ...
  const handleSaveFile = async () => {
    try {
      if (!ctx.filesystem) return;
      await ctx.filesystem.saveFile(new Blob([output], { type: 'text/css' }), 'formatted.css');
    } catch {
      // User cancelled
    }
  };
        ...
        <div className="flex flex-col gap-3 pb-2 pt-1">
          <label className="flex items-center gap-2 text-sm cursor-pointer whitespace-nowrap">
            <Switch
              ...
              onCheckedChange={(c) => updateOptions({ singleQuote: c })}
            />
            Single Quotes
          </label>
        </div>
        ...
        >
          <div className="relative min-h-[400px]">
            ...
            {error ? (
              ...
              <CodeEditor value={output} readOnly language="css" className="min-h-[400px]" />
            )}
          </div>
        </ToolField>
      </div>
